...
public class IoptsCurl {
	...
	private static final String BOUNDARY = "------------aia113jskyun1024";
	...
	private static final Map<String, Integer> optMap = Util.mapPut(
			new LinkedHashMap<String, Integer>(), "-E", 32, "--cert", 32, // <certificate[:password]> Client certificate file and password
			...
			"-c", 4, "--cookie-jar", 4, // FILE Write cookies to FILE after operation (H)
			...
			"-k", 31, "--insecure", 31, // Allow insecure server connections when using SSL
			"-L", 13, "--location", 13, // Follow redirects (H)
			"-m", 14, "--max-time", 14, // SECONDS Maximum time allowed for the transfer
			// "--no-keepalive", 15, // Disable keepalive use on the connection
			"-o", 16, "--output", 16, // FILE Write to FILE instead of stdout
			"-x", 17, "--proxy", 17, // [PROTOCOL://]HOST[:PORT] Use proxy on given port
			...
			"-A", 26, "--user-agent", 26, // STRING Send User-Agent STRING to server (H)
	...
	public IoptsCurl(String url) {
		this.url(url);
	}

	public final IoptsCurl opt(String... options) {
	...
	public final IoptsCurl url(String url) {
		return opt("--url", url);
	}

	public final IoptsCurl location() {
		return opt("-L");
	}

	public final IoptsCurl proxy(String host, int port) {
		return opt("-x", host + ":" + port);
	}

	public final IoptsCurl insecure() {
		return opt("-k");
	}

	public final IoptsCurl retry(int retry, float retryDelay, float retryMaxTime) {
		return opt("--retry", Integer.toString(retry), "--retry-delay", Float.toString(retryDelay), "--retry-max-time", Float.toString(retryMaxTime));
	}

	public final IoptsCurl timeout(float connectTimeoutSeconds, float readTimeoutSeconds) {
		return opt("--connect-timeout", Float.toString(connectTimeoutSeconds), "--max-time", Float.toString(readTimeoutSeconds));
	}

	public final IoptsCurl header(String headerLine) {
		return opt("-H", headerLine);
	}

	public final IoptsCurl headers(Map<String, ?> headers) {
	...
	public final IoptsCurl data(String data) {
		return data(data, false);
	}

	public final IoptsCurl data(String data, boolean raw) {
		return opt(raw ? "--data-raw" : "-d", data);
	}

	public final IoptsCurl data(IO input, boolean binary) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), input);
		return opt(binary ? "--data-binary" : "-d", "@" + key);
	}

	public final IoptsCurl data(String data, String charset) {
		return opt("--data-urlencode" + (charset != null ? "-" + charset : ""), data);
	}

	public final IoptsCurl form(String name, String content) {
		return opt("-F", name + "=" + content);
	}

	public final IoptsCurl form(String name, IO input) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), input);
		return opt("-F", name + "=@" + key);
	}

	public final IoptsCurl form(String formString) {
		return opt("--form-string", formString);
	}

	public final IoptsCurl cookieJar(String output) {
		return opt("-c", output);
	}

	public final IoptsCurl cookieJar(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
		return opt("-c", key);
	}

	public final IoptsCurl cookie(String input) {
		return opt("-b", input);
	}

	public final IoptsCurl cookie(IO input) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), input);
		return opt("-b", key);
	}

	public final IoptsCurl dumpHeader(String output) {
		return opt("-D", output);
	}

	public final IoptsCurl dumpHeader(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
		return opt("-D", key);
	}

	public final IoptsCurl cert(IO certificate, String password) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), certificate);
		return opt("-E", key + ":" + password);
	}

	// output can be an OutputStream/File/path_to_file
	public final IoptsCurl stderr(String output) {
		return opt("--stderr", output); 
	}

	public final IoptsCurl stderr(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
	...
	// output can be an OutputStream/File/path_to_file
	public final IoptsCurl output(String output) {
		return opt("-o", output); 
	}

	public final IoptsCurl output(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
		return opt("-o", key);
	}

	public final IoptsCurl io(String key, IO io) {
		iomap.put(key, io);
		return this;
	}
	...
	public static String dumpCookie(HttpCookie cookie) {
		...
		long expire = cookie.getMaxAge() <= 0 || cookie.getMaxAge() >= Integer.MAX_VALUE ? Integer.MAX_VALUE : cookie.getMaxAge() + System.currentTimeMillis() / 1000L;
		sb.append(cookie.getDomain()).append('\t').append("FALSE").append('\t').append(cookie.getPath()).append('\t').append(cookie.getSecure() ? "TRUE" : "FALSE").append('\t').append(expire)
				.append('\t').append(cookie.getName()).append('\t').append(cookie.getValue()).append('\n');
		return sb.toString();
	}
	...
	public final <T> T getStdout(Resolver<T> resolver, T fallback) {
		try {
			return resolver.resolve(httpCode, rawStdout);
	...
	public final <T> T exec(Resolver<T> resolver, T fallback) {
		...
		for (int i = 0, n = options.size(); i < n; i++) {
			...
			case 5: // --data DATA HTTP POST data (H)
				...
				if (data.startsWith("@"))
					data = Util.b2s(readInput(getIO(data.substring(1))), null, null).replaceAll("[\r\n]+", "");
			...
			case 53: // --data-urlencode
				...
				default: // name=content
					...
					for (Map.Entry<String, String> en : m.entrySet()) {
						try {
							en.setValue(URLEncoder.encode(en.getValue(), "UTF-8"));
						} catch (Exception ignored) {
						}
					}
					data = Util.join(m, "&", "=");
		...
		if (form.size() > 0) {
			...
			dataStr = !mergeData ? dataSb.toString() : Util.join(Util.split(dataSb.toString(), "&", "=", new LinkedHashMap<String, String>()), "&", "=");
			if (method == null)
				method = "POST";
		}
		if (method == null)
		...
		do {
			try {
						...
						for (Map.Entry<String, Util.Ref<String>> en : form.entrySet()) {
							...
							if (val.getInt() == 1) {
								...
								if (_1st.startsWith("@") || _1st.startsWith("<")) { // it's file
									...
									filename = _1st.startsWith("<") ? null : filename != null ? filename : f != null ? f.getAbsolutePath() : name;
				...
				try {
					...
					if (httpCode == 407 && proxy != Proxy.NO_PROXY && "https".equals(urlObj.getProtocol()) && headers.containsKey("Proxy-Authorization")) {
				...
				if (maxDownload <= 0 && bb != null && bb.length > 2) {
					if ((b0 = bb[0]) == 0x1F && bb[1] == (byte) 0x8B)
						is = new GZIPInputStream(new ByteArrayInputStream(bb)); // gzip
					if (b0 == 0x78 && ((b1 = bb[1]) == 0x01 || b1 == 0x5E || b1 == (byte) 0x9C || b1 == (byte) 0xDA))
						is = new InflaterInputStream(new ByteArrayInputStream(bb)); // deflate/zlib
				...
				if (dumpHeader != null)
					dumpHeader(responseHeaders.get(idx), dumpHeader);
				if (bb != null && bb.length > 0)
					writeOutput(output, bb, output == dumpHeader);
				if (lastEx != null)
					throw lastEx;
				...
				if (needRetry && retryLeft > 0 && retryDelay > 0)
					try {
						Thread.sleep((long) (retryDelay * 1000d));
					} catch (Exception ignored) {
	...
	private static void fillResponseHeaders(HttpURLConnection con, List<String[]> headers) {
		...
		String[] keys, values;
		Integer nkeys;
		if (responses != null && (nkeys = (Integer) Util.getField(responses, null, "nkeys", null, true)) != null && (keys = (String[]) Util.getField(responses, null, "keys", null, true)) != null
	...
	private static final HashSet<Class> RECOVERABLES = Util.listAdd(new HashSet<Class>(), (Class) Recoverable.class, ConnectException.class, HttpRetryException.class, SocketException.class,
			SocketTimeoutException.class, NoRouteToHostException.class);
	...
	final static class Util {
		...
		public static void logStderr(String msg, Object... args) {
			...
			System.out.println("[ERR] [" + new Date() + "] " + msg);
		}
		...
		public static byte[] readStream(InputStream is, int interruptOnSize, boolean close) {
			ByteArrayOutputStream bos = new ByteArrayOutputStream();
			int count = 0, c;
			while ((c = pipeStream(is, bos)) > 0 && (interruptOnSize <= 0 || count < interruptOnSize))
				count += c;
			if (c < 0)
		...
		public static Class<?> getClass(String className, ClassLoader cl) {
			try {
				return (cl != null ? cl : IoptsCurl.class.getClassLoader()).loadClass(className);
		...
		public static Object invoke(Object thiz, Class<?> cls, String methodName, boolean ignoreAccess, String signature, Object... args) {
			...
			try {
				...
				AccessibleObject acc;
				if (ignoreAccess && !(acc = (AccessibleObject) m).isAccessible())
					acc.setAccessible(true);
		...
		private static final Map<String, Object> primaryTypes = newMap(byte.class, 'B', char.class, 'C', double.class, 'D', float.class, 'F', int.class, 'I', long.class, 'J', short.class, 'S',
				void.class, 'V', boolean.class, 'Z');
